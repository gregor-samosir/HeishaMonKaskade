...
void build_heatpump_command(char *topic, char *msg)
{
  ...
  if (Topics::SET1.compare(topic) == 0)
  {
    ...
    sprintf(log_msg, "COMMAND: SET1 %s: %d", topic, set_byte);
  ...
  else if (Topics::SET3.compare(topic) == 0)
  {
    ...
    sprintf(log_msg, "COMMAND: SET3 %s: %d", topic, set_byte / 8 - 1);
    mainCommand[set_pos] = set_byte;
  }
  ...
  else if (Topics::SET5.compare(topic) == 0)
  {
    ...
    sprintf(log_msg, "COMMAND: SET5 %s: %d", topic, set_byte - 128);
    mainCommand[set_pos] = set_byte;
  }
  ...
  else if (Topics::SET6.compare(topic) == 0)
  {
    ...
    set_pos = 39;
    set_byte = msg_int + 128;
    sprintf(log_msg, "COMMAND: SET6 %s: %d", topic, set_byte - 128);
    mainCommand[set_pos] = set_byte;
  }
  ...
  else if (Topics::SET7.compare(topic) == 0)
  {
    ...
    set_pos = 40;
    set_byte = msg_int + 128;
    sprintf(log_msg, "COMMAND: SET7 %s: %d", topic, set_byte - 128);
    mainCommand[set_pos] = set_byte;
  }
  ...
  else if (Topics::SET8.compare(topic) == 0)
  {
    ...
    set_pos = 41;
    set_byte = msg_int + 128;
    sprintf(log_msg, "COMMAND: SET8 %s: %d", topic, set_byte - 128);
    mainCommand[set_pos] = set_byte;
  }
  ...
  else if (Topics::SET10.compare(topic) == 0)
  {
    ...
    sprintf(log_msg, "COMMAND: SET10 %s: %d", topic, set_byte);
    mainCommand[set_pos] = set_byte;
  }
  ...
  else if (Topics::SET12.compare(topic) == 0)
  {
    ...
    sprintf(log_msg, "COMMAND: SET12 %s: %d", topic, set_byte);
    mainCommand[set_pos] = set_byte;
  }
  ...
  else if (Topics::SET13.compare(topic) == 0)
  {
    ...
    sprintf(log_msg, "COMMAND: SET13 %s: %d", topic, set_byte);
    mainCommand[set_pos] = set_byte;
  }
  ...
  else if (Topics::SET2.compare(topic) == 0)
  {
    ...
    sprintf(log_msg, "COMMAND: SET2 %s: %d", topic, set_byte);
    mainCommand[set_pos] = set_byte;
  }
  ...
  else if (Topics::SET4.compare(topic) == 0)
  {
    ...
    set_pos = 7;
    set_byte = (msg_int) + 73;
    sprintf(log_msg, "COMMAND: SET4 %s: %d", topic, (set_byte - 73));
    mainCommand[set_pos] = set_byte;
  }
  ...
  else if (Topics::SET9.compare(topic) == 0)
  {
    ...
    sprintf(log_msg, "COMMAND: SET9 %s: %d", topic, set_byte);
    mainCommand[set_pos] = set_byte;
  }
  ...
  else if (Topics::SET11.compare(topic) == 0)
  {
    ...
    set_pos = 42;
    set_byte = msg_int + 128;
    sprintf(log_msg, "COMMAND: SET11 %s: %d", topic, set_byte - 128);
    mainCommand[set_pos] = set_byte;
  }
  ...
  else if (Topics::SET14.compare(topic) == 0)
  {
    ...
    sprintf(log_msg, "COMMAND: SET14 %s: %d", topic, set_byte);
    mainCommand[set_pos] = set_byte;
  }
  ...
  else if (Topics::SET16.compare(topic) == 0)
  {
    ...
    set_pos = 84;
    set_byte = msg_int + 128;
    sprintf(log_msg, "COMMAND: SET16 %s: %d", topic, set_byte - 128);
    mainCommand[set_pos] = set_byte;
  }
  // set cool delta 1-15
  else if (Topics::SET17.compare(topic) == 0)
  {
    ...
    set_pos = 94;
    set_byte = msg_int + 128;
    sprintf(log_msg, "COMMAND: SET17 %s: %d", topic, set_byte - 128);
    mainCommand[set_pos] = set_byte;
  }
  ...
  else if (Topics::SET18.compare(topic) == 0)
  {
    ...
    set_pos = 99;
    set_byte = msg_int + 128;
    sprintf(log_msg, "COMMAND: SET18 %s: %d", topic, set_byte - 128);
    mainCommand[set_pos] = set_byte;
  }
  ...
  else if (Topics::SET19.compare(topic) == 0)
  {
    ...
    set_pos = 98;
    set_byte = msg_int + 1;
    sprintf(log_msg, "COMMAND: SET19 %s: %d", topic, set_byte - 1);
    mainCommand[set_pos] = set_byte;
  }
