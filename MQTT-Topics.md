...
TOP70 | Sterilization_Temp | Sterilisation Temperature (°C)
TOP71 | Sterilization_Max_Time | Sterilisation maximum time (minutes)
...
> TOP59 already read `Free` on both stages before the run — that was the owner's
> own test release at the control panel, which makes the run a confirmation of
> the mapping: byte 9 showed exactly what the panel had been set to.
>
